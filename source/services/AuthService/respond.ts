...
interface ResponseCompleteCallback {
  (error: Error, result: ResponseResult): void
}
...
export default function respond(response: Response, callback: ResponseCompleteCallback = () => {}): void {
  ...
  if (window.opener == null) {
    callback(null, { response: response, success: false });
  } else {
    window.opener.postMessage(JSON.stringify(message), window.location.origin);
    callback(null, { response: response, success: true });
  }
}
